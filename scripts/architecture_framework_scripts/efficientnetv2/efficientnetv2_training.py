...
SAVE_DIR = "models/efficientnetv2/trained"
...
transform = transforms.Compose([
    transforms.Resize((224, 224)),  # EfficientNetV2 input size
    transforms.ToTensor(),
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
    ...
    # Track best model
    best_f1 = 0.0
    best_model_path = os.path.join(SAVE_DIR, f"efficientnetv2_plantvillage_best_fold{fold+1}.pth")
    ...
    epoch_df.to_csv(os.path.join(SAVE_DIR, f"efficientnetv2_plantvillage_fold{fold+1}_epochs.csv"), index=False)
...
df_results.to_csv(os.path.join(SAVE_DIR, f"efficientnetv2_plantvillage_results_{timestamp}.csv"), index=False)
