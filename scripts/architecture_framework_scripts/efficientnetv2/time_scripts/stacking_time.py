import os
import torch
import torch.nn as nn # For nn.Linear if needed, models.efficientnet_v2_s handles its own layers
...
# Relative paths (USER ACTION: Verify these paths match your project structure)
...
META_MODEL_PATH = os.path.join("models", "efficientnetv2", "meta_learner", "efficientnet_meta_learner.pkl")
...
# --- Load DL Model (EfficientNetV2 base model WITH Warm-up) ---
def load_efficientnet_base_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
    print(f"[INFO] Loading EfficientNetV2-S base model (WITH WARM-UP) for {num_classes_config} classes...")
    model = models.efficientnet_v2_s(weights=None) # Load architecture
...
def load_sklearn_model(path, model_type="Model"): # Generic loader for KNN and Meta
...
def run_stacking_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_dl_model_path, current_knn_model_path, current_meta_model_path):
    ...
    try:
        dl_model = load_efficientnet_base_model_with_warmup(current_dl_model_path, NUM_CLASSES, DEVICE)
    ...
    for img_path, current_true_label in tqdm(zip(image_paths, true_labels_all), total=len(true_labels_all), desc=f"Inferring on {dataset_name_key}"):
        try:
            ...
            with torch.no_grad():
                ...
                if DEVICE.type == 'cuda': torch.cuda.synchronize()
                # Using F.softmax as in original efficientnetv2_stacking_inference.py
            ...
            # extract_glcm_features takes image_path as per RegNet template
