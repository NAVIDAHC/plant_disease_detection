...
import joblib # Using joblib as in the RegNet example
import cv2
from skimage.feature import graycomatrix, graycoprops
import time

# --- Config ---
ARCH_NAME = "efficientnetv2" # Changed from "regnet"
INFERENCE_TYPE_TAG = "hierarchical"

NUM_CLASSES = None # Dynamically determined from first dataset
...
# Updated DL_MODEL_PATH for EfficientNetV2
DL_MODEL_PATH = os.path.join("models", "efficientnetv2", "trained", "efficientnetv2_best_model.pth")
KNN_MODEL_PATH = os.path.join("models", "glcm_knn", "trained", "glcm_knn.pkl") # As confirmed by user

DATASET1_PATH = os.path.join("dataset", "plantvillage", "test")
DATASET2_PATH = os.path.join("dataset", "plantdoc", "test")

IMAGE_SIZE = (224, 224) # Standard for these models
TRANSFORM = transforms.Compose([ # Standard transform
...
# --- Load DL Model (EfficientNetV2 WITH Warm-up) ---
def load_efficientnet_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
    # This function is adapted for EfficientNetV2 from the RegNet template's style
    print(f"[INFO] Loading {ARCH_NAME} model (WITH WARM-UP) for {num_classes_config} classes...")
    model = models.efficientnet_v2_s(weights=None) # Using efficientnet_v2_s

    # Modify classifier for EfficientNetV2-S
    if isinstance(model.classifier, nn.Sequential) and len(model.classifier) > 1 and isinstance(model.classifier[1], nn.Linear):
        ...
        print(f"[WARN] {ARCH_NAME} classifier structure not as nn.Sequential with Linear at index 1. Attempting to replace model.classifier directly.")
        try:
            num_features = model.classifier.in_features
            model.classifier = nn.Linear(num_features, num_classes_config)
        except AttributeError:
            raise RuntimeError(f"Could not determine in_features or replace classifier for {ARCH_NAME}. Check model structure.")
    ...
    if device_to_use.type == 'cuda':
        ...
        try:
            # Using min(4, num_classes_config if num_classes_config else 2) to avoid error if num_classes_config is None during an unexpected call
...
# --- GLCM Feature Extractor (Identical to the working RegNet hierarchical script) ---
def extract_glcm_features(image_path):
    img = cv2.imread(image_path)
    if img is None:
        try:
            pil_img = Image.open(image_path).convert('L') # Try with PIL if cv2 fails
    ...
    if np.all(gray == gray[0,0]): # Check for blank image
        ...
        return np.zeros((1,6)) # Expected 6 features from the RegNet template's GLCM

    # Using distances=[1], angles=[0] and 6 properties as per RegNet template
    ...
    ]).reshape(1, -1) # Reshape to (1, 6)

# --- Load KNN Model ---
def load_knn_model(path): # Taken from RegNet template
    print(f"[INFO] Loading KNN model from: {path}")
    try:
        knn = joblib.load(path) # RegNet template used joblib
...
def get_image_paths_and_labels(root_dir, class_names_config_ref): # Taken from RegNet template
    image_paths, labels = [], []
    ...
    global NUM_CLASSES, CLASS_NAMES, HEALTHY_CLASS_INDEX # Allow modification
...
def save_per_dataset_classification_results(report, conf_mat, arch_conf, inf_type_conf, ds_name, report_cls_names): # report_cls_names should be global CLASS_NAMES
...
def run_hierarchical_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_dl_model_path): # Name matches RegNet template
    ...
    if NUM_CLASSES is None or CLASS_NAMES is None or HEALTHY_CLASS_INDEX is None:
        ...
        return None
    if HEALTHY_CLASS_INDEX == -1:
        print(f"[ERROR] Healthy class '{HEALTHY_CLASS_NAME}' not found in CLASS_NAMES. Cannot proceed with {dataset_name_key}.")
        return None

    try:
        # Use the correct loader for EfficientNetV2
        dl_model = load_efficientnet_model_with_warmup(current_dl_model_path, NUM_CLASSES, DEVICE)
    ...
    for img_path, current_true_label in tqdm(zip(image_paths, true_labels_all), total=len(true_labels_all), desc=f"Inferring on {dataset_name_key}"):
        try:
            ...
            # GLCM features extracted first (using image_path directly in extract_glcm_features as per RegNet template)
            glcm_features_np = extract_glcm_features(img_path) # This will do cv2.imread
...
if __name__ == "__main__":
    ...
    # Check model paths before starting
    if not os.path.exists(DL_MODEL_PATH) or not DL_MODEL_PATH.lower().endswith((".pth", ".pt")):
        print(f"[FATAL] DL_MODEL_PATH ('{DL_MODEL_PATH}') invalid or not a .pth/.pt file. Update path. Exiting.")
        exit()
    ...
    if first_valid_ds_path_for_class_init:
        try:
            print(f"[INFO] Initializing class information from first valid dataset: {first_valid_ds_path_for_class_init}")
            _, _, _ = get_image_paths_and_labels(first_valid_ds_path_for_class_init, None)
            if NUM_CLASSES is None or CLASS_NAMES is None or HEALTHY_CLASS_INDEX is None:
                 raise RuntimeError("Class/Healthy index information not set after initial dataset load.")
            if HEALTHY_CLASS_INDEX == -1:
        ...
        print("[FATAL] No valid dataset paths found in datasets_to_process. Cannot determine class information. Exiting.")
        exit()
    ...
    for dataset_name_key, dataset_actual_path in datasets_to_process.items():
        ...
        timing_result = run_hierarchical_inference_on_dataset( # Calling the correct function name
            dataset_actual_path,
            dataset_name_key,
            ARCH_NAME,
            INFERENCE_TYPE_TAG,
            DL_MODEL_PATH # Pass the DL model path for EfficientNetV2
        )
        if timing_result:
