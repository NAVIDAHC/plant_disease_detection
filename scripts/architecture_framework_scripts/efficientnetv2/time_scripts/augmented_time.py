...
# import torch.nn.functional as F # Not explicitly used, can be removed if not needed by AugmentedEfficientNet internals
import joblib # For loading KNN model, consistent with RegNet example
import pandas as pd
import numpy as np
...
# --- Config ---
ARCH_NAME = "efficientnetv2"
INFERENCE_TYPE_TAG = "augmented"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {DEVICE}")

# Standardized dataset paths (assuming these are your test sets)
DATASET1_PATH = os.path.join("dataset", "plantvillage", "test")
DATASET2_PATH = os.path.join("dataset", "plantdoc", "test")

# Standardized model paths (USER ACTION: Ensure these paths are correct for your files)
KNN_MODEL_PATH = os.path.join("models", "glcm_knn", "trained", "glcm_knn.pkl")
AUGMENTED_MODEL_PATH = os.path.join("models", "efficientnetv2", "augmented", "efficientnetv2_augmented.pth")
print(f"[INFO] Using Augmented EfficientNetV2 model path: {AUGMENTED_MODEL_PATH}")
print(f"[INFO] Using KNN model path: {KNN_MODEL_PATH}")


NUM_CLASSES = None # Dynamically determined from the first loaded dataset
CLASS_NAMES = None # Dynamically determined

IMAGE_SIZE = (224, 224) # For EfficientNetV2
...
# --- GLCM extractor (consistent with RegNet working version) ---
def extract_glcm_features(image_path_or_cv2_img):
    if isinstance(image_path_or_cv2_img, str): # if path is given
        img_cv2 = cv2.imread(image_path_or_cv2_img)
        if img_cv2 is None:
            try: # Fallback to PIL if cv2 fails for path
    ...
    elif isinstance(image_path_or_cv2_img, np.ndarray): # if cv2 image is given
    ...
    if np.all(gray == gray[0,0]):
        # print(f"[WARN] Image appears blank/uniform. GLCM returning zeros.")
        return np.zeros((1,6)) # 6 features matching RegNet GLCM
    ...
    return np.array(features).reshape(1, -1) # Shape (1, 6)

# --- Augmented Model Definition (from original script) ---
class AugmentedEfficientNet(nn.Module):
    def __init__(self, num_classes_global=9, knn_feature_dim_global=9): # Use global defaults
        super().__init__()
        self.backbone = models.efficientnet_v2_s(weights=None) # Load architecture
        # Determine feature_dim before modifying classifier
        # Original script did this after loading weights, but better before if weights are for full model.
        # If weights are only for backbone, this is fine.
        # Assuming MODEL_PATH contains state_dict for the *entire* AugmentedEfficientNet
        temp_classifier_out_features = self.backbone.classifier[1].out_features # Store original out_features
        self.feature_dim = self.backbone.classifier[1].in_features
        self.backbone.classifier = nn.Identity() # Remove original classifier to get features

        self.knn_feature_dim = knn_feature_dim_global

        self.classifier = nn.Sequential(
            nn.Linear(self.feature_dim + self.knn_feature_dim, 256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, num_classes_global)
        )

    def forward(self, image_tensor, knn_proba_vector):
        image_features = self.backbone(image_tensor)
        # Ensure knn_proba_vector is 2D: (batch_size, knn_feature_dim)
        if knn_proba_vector.ndim == 1:
            knn_proba_vector = knn_proba_vector.unsqueeze(0)
        if image_features.shape[0] != knn_proba_vector.shape[0]:
             raise ValueError(f"Batch size mismatch: image_features {image_features.shape[0]}, knn_vector {knn_proba_vector.shape[0]}")
    ...
    def load_and_prepare_model(self, model_state_dict_path, device_to_use):
        print(f"[INFO] Loading AugmentedEfficientNet state_dict from: {model_state_dict_path}")
        try:
            # This loads the state_dict for the *entire* AugmentedEfficientNet, including backbone and new classifier
            ...
            if isinstance(checkpoint, dict) and ('model_state_dict' in checkpoint or 'state_dict' in checkpoint):
                ...
                print(f"[INFO] Loaded AugmentedEfficientNet state_dict from checkpoint key '{key_to_use}'.")
            else:
                self.load_state_dict(checkpoint) # Assumed raw state_dict for the AugmentedEfficientNet
                print("[INFO] Loaded AugmentedEfficientNet state_dict directly (assumed raw).")

        except FileNotFoundError:
            print(f"[ERROR] AugmentedEfficientNet state_dict not found at: {model_state_dict_path}")
            raise
        except Exception as e:
            print(f"[ERROR] Failed to load AugmentedEfficientNet state_dict: {e}")
            raise

        self.to(device_to_use).eval()

        # Warm-up for the entire AugmentedEfficientNet model
        if device_to_use.type == 'cuda':
            print("[INFO] Performing GPU warm-up for AugmentedEfficientNet model...")
            try:
                dummy_img_tensor = torch.randn(1, 3, IMAGE_SIZE[0], IMAGE_SIZE[1]).to(device_to_use)
                dummy_knn_tensor = torch.randn(1, self.knn_feature_dim).to(device_to_use) # Use self.knn_feature_dim
                with torch.no_grad():
                    for _ in range(5):
                        _ = self.forward(dummy_img_tensor, dummy_knn_tensor)
                torch.cuda.synchronize()
                print("[INFO] AugmentedEfficientNet GPU warm-up complete.")
            except Exception as e:
                print(f"[WARN] Error during AugmentedEfficientNet warm-up: {e}")
        return self


# --- Load KNN Model ---
def load_knn_model(path): # Copied from RegNet hierarchical for consistency
...
def get_image_paths_and_labels(root_dir, class_names_config_ref): # Adapted from other scripts
    image_paths, labels = [], []
    ...
    if class_names_config_ref is None: # First dataset being processed
...
def run_augmented_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_augmented_model_path):
    ...
    if NUM_CLASSES is None or CLASS_NAMES is None: # Crucial check
        print(f"[ERROR] Global class information (NUM_CLASSES, CLASS_NAMES) not set. Cannot proceed with {dataset_name_key}.")
        return None

    try:
        # knn_feature_dim for AugmentedEfficientNet should match output of knn.predict_proba()
        # which is typically the number of classes in the KNN model's training data.
        # Assuming it's NUM_CLASSES (global). If KNN outputs fixed 6 features, change here.
        # The GLCM here produces 6 features, KNN predict_proba on that will yield NUM_CLASSES probabilities.
        augmented_model_instance = AugmentedEfficientNet(num_classes_global=NUM_CLASSES, knn_feature_dim_global=NUM_CLASSES)
    ...
    for img_path, current_true_label in tqdm(zip(image_paths, true_labels_all), total=len(true_labels_all), desc=f"Inferring on {dataset_name_key}"):
        try:
            ...
            # 1. GLCM feature extraction (path directly to GLCM)
            glcm_feat_np = extract_glcm_features(img_path) # Expected shape (1,6)

            # 2. KNN probability prediction
            knn_proba_np = knn_model.predict_proba(glcm_feat_np).flatten() # Flatten to 1D array of NUM_CLASSES length
            ...
            # 5. Forward pass through AugmentedEfficientNet
            with torch.no_grad():
