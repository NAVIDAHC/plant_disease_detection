...
META_MODEL_PATH = "models/resnet50/meta_learner/resnet50_meta_learner.pkl"
...
def run_stacking_inference(dataset_path, dataset_name):
    ...
    for path, true_label in tqdm(zip(image_paths, labels), total=len(labels)):
        try:
            ...
            with torch.no_grad():
                dl_probs = torch.softmax(model(input_tensor), dim=1).cpu().numpy().flatten()
    ...
    # --- Save Results ---
    out_dir = f"results/resnet50/stacking/{dataset_name}"
    ...
    plt.ylabel("Actual")
    plt.title(f"{ARCH_NAME.upper()} Stacking Confusion Matrix")
    plt.tight_layout()
