...
def run_hierarchical_inference(dataset_path, dataset_name):
    ...
    for path, true_label in tqdm(zip(image_paths, labels), total=len(labels)):
        try:
            ...
            if knn_pred == HEALTHY_CLASS_INDEX:
                ...
                with torch.no_grad():
                    dl_pred = torch.argmax(model(input_tensor), dim=1).item()
                final_pred = dl_pred
    ...
    out_dir = f"results/regnet/hierarchical/{dataset_name}"
    ...
    plt.ylabel("Actual")
    plt.title(f"{ARCH_NAME.upper()} Hierarchical Confusion Matrix")
    plt.tight_layout()
