...
import joblib # Using joblib for consistency with RegNet example
import cv2
...
# Relative paths (mostly good in original, ensuring os.path.join)
DL_MODEL_PATH = os.path.join("models", "swin_transformer", "trained", "swin_transformer_best_model.pth")
...
IMAGE_SIZE = (224, 224) # Swin Transformer often uses 224x224 or 384x384
...
# --- Load DL Model (Swin Transformer WITH Warm-up) ---
...
# --- GLCM Feature Extractor (consistent with RegNet hierarchical) ---
...
def run_hierarchical_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_dl_model_path):
    ...
    try:
        # Load DL model (Swin Transformer) with warm-up
        dl_model = load_swin_transformer_model_with_warmup(current_dl_model_path, NUM_CLASSES, DEVICE)
    ...
    for img_path, current_true_label in tqdm(zip(image_paths, true_labels_all), total=len(true_labels_all), desc=f"Inferring on {dataset_name_key}"):
        try:
            ...
            glcm_features_np = extract_glcm_features(img_path)
...
if __name__ == "__main__":
    ...
    if first_valid_ds_path_for_class_init:
        try:
            ...
            if HEALTHY_CLASS_INDEX == -1:
                print(f"[FATAL] Healthy class '{HEALTHY_CLASS_NAME}' not found in first dataset ({CLASS_NAMES}). Hierarchical logic error. Exiting.")
                exit()
        except Exception as e:
            print(f"[FATAL] Could not init class info from {first_valid_ds_path_for_class_init}. Error: {e}. Exiting.")
            exit()
    else:
        print("[FATAL] No valid dataset paths. Cannot determine class info. Exiting.")
        exit()
    ...
    for dataset_name_key, dataset_actual_path in datasets_to_process.items():
        ...
        timing_result = run_hierarchical_inference_on_dataset(
            ...
            ARCH_NAME,
            INFERENCE_TYPE_TAG,
            DL_MODEL_PATH # Pass the Swin Transformer model path
        )
        if timing_result:
