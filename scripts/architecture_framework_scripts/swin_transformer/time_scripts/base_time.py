...
# Relative paths (already good in original)
DATASET1_PATH = os.path.join("dataset", "plantvillage", "test")
DATASET2_PATH = os.path.join("dataset", "plantdoc", "test")
MODEL_PATH = os.path.join("models", "swin_transformer", "trained", "swin_transformer_best_model.pth")
...
def load_swin_transformer_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
    ...
    # If MODEL_PATH is only a fine-tuned head, you might start with pretrained backbone:
    ...
    if hasattr(model, 'head') and isinstance(model.head, nn.Linear):
        ...
        model.head = nn.Linear(num_features, num_classes_config)
    else:
        raise RuntimeError(f"Could not modify head of {ARCH_NAME}. Expected model.head to be nn.Linear.")
    ...
    try:
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
         ...
         raise
    except Exception as e: # Catch other loading issues like key mismatches
    ...
    if device_to_use.type == 'cuda':
        ...
        try:
            dummy_batch_size = min(BATCH_SIZE, 4, num_classes_config if num_classes_config else 4)
...
def load_dataset_for_inference(dataset_path, batch_size_config, transform_config):
    ...
    if NUM_CLASSES is None: # First time loading a dataset
        ...
        CLASS_NAMES = list(dataset.classes) # Ensure it's a list
    ...
    dataloader = DataLoader(dataset,
                            ...
                            shuffle=False,
                            num_workers=min(os.cpu_count(), 2), # Adjusted num_workers
                            pin_memory=True if DEVICE.type == 'cuda' else False)
    print(f"[INFO] Loaded dataset from {dataset_path} with {len(dataset)} images in {len(dataset.classes)} classes.")
    return dataloader, len(dataset), list(dataset.classes) # Return current dataset's class names
...
def evaluate_model_with_timing(model, dataloader, device_to_use, current_dataset_name_for_log, current_class_names_list):
    ...
    # Ensure labels for report are based on the actual number of classes in current_class_names_list
...
def run_base_inference_on_dataset(dataset_actual_path, dataset_name_key, arch_name_config, inference_type_tag_config, model_weights_path):
    ...
    try:
        ...
        # Use global CLASS_NAMES for consistency in reports if model is trained on one set of classes
        # but ds_specific_class_names can be used if you need to log or verify against the current dataset's exact classes
    ...
    # Load model (includes warm-up). Model is adapted to global NUM_CLASSES determined from first dataset.
    try:
        if NUM_CLASSES is None: # Should have been set by load_dataset_for_inference's first call
             raise RuntimeError("NUM_CLASSES has not been determined before model loading.")
        model_to_eval = load_swin_transformer_model_with_warmup(model_weights_path, NUM_CLASSES, DEVICE)
    except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    if first_valid_ds_path:
        try:
            ...
            # This call will set global NUM_CLASSES and CLASS_NAMES
    ...
    for dataset_name_key, dataset_actual_path in datasets_to_process.items():
        ...
        timing_result = run_base_inference_on_dataset(
            ...
            ARCH_NAME,
            INFERENCE_TYPE_TAG,
            MODEL_PATH # Pass the model path
        )
        if timing_result:
