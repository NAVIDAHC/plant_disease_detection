...
META_MODEL_PATH = os.path.join("models", "swin_transformer", "meta_learner", "swin_transformer_meta_learner.pkl")
...
IMAGE_SIZE = (224, 224) # Standard for Swin Transformer
...
# --- Load DL Model (Swin Transformer base model WITH Warm-up) ---
def load_swin_transformer_base_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
...
def get_image_paths_and_labels(root_dir, class_names_config_ref):
    ...
    for class_dir_name in current_dataset_class_names:
        if class_dir_name not in CLASS_NAMES: # Check against global list
...
def run_stacking_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config,
    ...
    try:
        # Load base DL model (Swin Transformer) with warm-up
        dl_model = load_swin_transformer_base_model_with_warmup(current_dl_model_path, NUM_CLASSES, DEVICE)
