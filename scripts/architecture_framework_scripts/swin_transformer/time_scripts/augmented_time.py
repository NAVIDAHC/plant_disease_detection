...
AUGMENTED_MODEL_PATH = os.path.join("models", "swin_transformer", "augmented", "swin_transformer_augmented.pth")
print(f"[INFO] Using Augmented Swin Transformer model path: {AUGMENTED_MODEL_PATH}")
...
IMAGE_SIZE = (224, 224) # For Swin Transformer
...
# --- Augmented Swin Model Definition (from original script) ---
class AugmentedSwin(nn.Module):
    def __init__(self, num_classes_global=9, knn_feature_dim_global=9): # Default to global NUM_CLASSES
        super().__init__()
        # Load Swin-T backbone. `weights=None` if MODEL_PATH for AugmentedSwin contains full model.
        # Original script used `weights=models.Swin_T_Weights.DEFAULT` which loads ImageNet weights.
        # If your augmented model was trained from scratch or with custom backbone weights, use None.
        # If it was fine-tuned from ImageNet-pretrained Swin, use DEFAULT and load only your classifier.
        # For consistency with loading the *entire* AugmentedSwin state_dict, using weights=None here.
        self.backbone = models.swin_t(weights=None)
        self.backbone.head = nn.Identity() # Remove original head to get features

        # Dynamically determine feature dimension from backbone (as in original)
        with torch.no_grad(): # This needs to run on CPU if model not yet on device
            dummy_cpu = torch.randn(1, 3, IMAGE_SIZE[0], IMAGE_SIZE[1])
            feature_dim = self.backbone(dummy_cpu).shape[1]
    ...
    def forward(self, img_tensor, knn_feature_tensor):
        image_features = self.backbone(img_tensor)
        if knn_feature_tensor.ndim == 1:
    ...
    def load_and_prepare_model(self, model_state_dict_path, device_to_use):
        print(f"[INFO] Loading AugmentedSwin state_dict from: {model_state_dict_path}")
        try:
            ...
            if isinstance(checkpoint, dict) and ('model_state_dict' in checkpoint or 'state_dict' in checkpoint):
                ...
                print(f"[INFO] Loaded AugmentedSwin state_dict from checkpoint key '{key_to_use}'.")
            else:
                self.load_state_dict(checkpoint)
                print("[INFO] Loaded AugmentedSwin state_dict directly (assumed raw).")
        except FileNotFoundError:
            print(f"[ERROR] AugmentedSwin state_dict not found at: {model_state_dict_path}")
            raise
        except Exception as e:
            print(f"[ERROR] Failed to load AugmentedSwin state_dict: {e}")
            raise

        self.to(device_to_use).eval()

        if device_to_use.type == 'cuda':
            print("[INFO] Performing GPU warm-up for AugmentedSwin model...")
            try:
                ...
                print("[INFO] AugmentedSwin GPU warm-up complete.")
            except Exception as e:
                print(f"[WARN] Error during AugmentedSwin warm-up: {e}")
        return self
...
def run_augmented_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_augmented_model_path):
    ...
    try:
        # The AugmentedSwin class expects num_classes_global and knn_feature_dim_global.
        # Assuming knn_feature_dim_global is NUM_CLASSES because knn.predict_proba outputs NUM_CLASSES probabilities.
        augmented_model_instance = AugmentedSwin(num_classes_global=NUM_CLASSES, knn_feature_dim_global=NUM_CLASSES)
    ...
    for img_path, current_true_label in tqdm(zip(image_paths, true_labels_all), total=len(true_labels_all), desc=f"Inferring on {dataset_name_key}"):
        try:
            ...
            knn_proba_np = knn_model.predict_proba(glcm_feat_np).flatten()
