...
SAVE_DIR = r"C:\Users\User\Desktop\ivan files\plant_disease_detection\models\architectures_data augmented\swin"
...
transform = transforms.Compose([
    transforms.Resize((224, 224)),  # Swin-T input size
    transforms.ToTensor(),
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
    ...
    model.head = nn.Linear(num_features, len(full_dataset.classes))
    ...
    # Track best model
    best_f1 = 0.0
    best_model_path = os.path.join(SAVE_DIR, f"swin_t_plantvillage_best_fold{fold+1}.pth")
    ...
    epoch_df.to_csv(os.path.join(SAVE_DIR, f"swin_t_plantvillage_fold{fold+1}_epochs.csv"), index=False)
...
df_results.to_csv(os.path.join(SAVE_DIR, f"swin_t_plantvillage_results_{timestamp}.csv"), index=False)
