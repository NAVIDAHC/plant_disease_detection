...
IMAGE_SIZE = (224, 224) # ConvNeXt typically uses 224x224
...
# --- Load DL Model (ConvNeXt WITH Warm-up) ---
def load_convnext_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
    ...
    # Modify the classifier for ConvNeXt_Tiny (original script modified model.classifier[2])
    ...
    else: # Fallback if structure differs slightly
        try:
            if isinstance(model.classifier, nn.Linear): # If classifier is a single Linear layer
                 num_features = model.classifier.in_features
                 model.classifier = nn.Linear(num_features, num_classes_config)
            elif isinstance(model.classifier, nn.Sequential) and isinstance(model.classifier[-1], nn.Linear): # If last layer in Seq is Linear
...
def run_hierarchical_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_dl_model_path):
    ...
    try:
        # Load DL model (ConvNeXt) with warm-up
        dl_model = load_convnext_model_with_warmup(current_dl_model_path, NUM_CLASSES, DEVICE)
...
if __name__ == "__main__":
    ...
    # Now global class info is set for model loading and logic
    ...
    for dataset_name_key, dataset_actual_path in datasets_to_process.items():
        ...
        timing_result = run_hierarchical_inference_on_dataset(
            ...
            ARCH_NAME,
            INFERENCE_TYPE_TAG,
            DL_MODEL_PATH # Pass the ConvNeXt model path
        )
        if timing_result:
