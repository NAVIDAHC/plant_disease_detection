...
MODEL_PATH = os.path.join("models", "convnext", "trained", "convnext_best_model.pth")
print(f"[INFO] Using ConvNeXt Model Path: {MODEL_PATH}")
...
def load_convnext_model_with_warmup(model_weights_path, num_classes_config, device_to_use):
    ...
    # Original script used models.ConvNeXt_Tiny_Weights.DEFAULT then loaded state_dict.
    ...
    # The classifier in convnext_tiny is a Sequential module, and the Linear layer is typically the last one.
    # Original script modified model.classifier[2]
    ...
    else: # Fallback if structure differs slightly, e.g. if classifier has fewer layers
        try:
            # Attempt to access the last linear layer if it's directly accessible or last in a shorter sequence
...
def load_dataset_for_inference(dataset_path, batch_size_config, transform_config):
    ...
    dataloader = DataLoader(dataset,
                            ...
                            shuffle=False,
                            num_workers=min(os.cpu_count(), 2), # Adjusted num_workers from original 0 for potential speedup
...
def run_base_inference_on_dataset(dataset_actual_path, dataset_name_key, arch_name_config, inference_type_tag_config, model_weights_path):
    ...
    try:
        ...
        model_to_eval = load_convnext_model_with_warmup(model_weights_path, NUM_CLASSES, DEVICE)
    except FileNotFoundError:
        print(f"[ERROR] Cannot proceed: Model weights not found at {model_weights_path}")
        return None
    except RuntimeError as e: # Catch specific errors from model loading like classifier modification
