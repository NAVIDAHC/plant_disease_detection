...
# import torch.nn.functional as F # Not explicitly used, can remove
...
AUGMENTED_MODEL_PATH = os.path.join("models", "convnext", "augmented", "convnext_augmented.pth")
print(f"[INFO] Using Augmented ConvNeXt model path: {AUGMENTED_MODEL_PATH}")
...
IMAGE_SIZE = (224, 224) # For ConvNeXt
...
# --- Augmented ConvNeXt Model Definition (adapted from original script) ---
class AugmentedConvNeXt(nn.Module):
    def __init__(self, num_classes_global_config=9, knn_feature_dim_global_config=9):
        ...
        # Original script used weights=models.ConvNeXt_Tiny_Weights.DEFAULT.
        # If AUGMENTED_MODEL_PATH contains the full model state (including backbone),
        # then weights=None is appropriate here.
        self.backbone = models.convnext_tiny(weights=None)

        # To get features, the original final layer (model.classifier[2]) is replaced.
        # First, get the feature dimension before replacing it.
        if hasattr(self.backbone, 'classifier') and isinstance(self.backbone.classifier, nn.Sequential) and \
           len(self.backbone.classifier) > 2 and isinstance(self.backbone.classifier[2], nn.Linear):
            self.feature_dim = self.backbone.classifier[2].in_features
            self.backbone.classifier[2] = nn.Identity() # Replace the linear layer
        else: # Fallback for slightly different structures
            try:
                if isinstance(self.backbone.classifier[-1], nn.Linear):
                    self.feature_dim = self.backbone.classifier[-1].in_features
                    self.backbone.classifier[-1] = nn.Identity()
                else:
                    raise AttributeError("Cannot find Linear layer to replace in ConvNeXt classifier.")
                print(f"[WARN] {ARCH_NAME} classifier structure modified using a fallback for feature extraction.")
            except (AttributeError, IndexError) as e:
                 # If dynamic feature extraction fails, try with a typical value for convnext_tiny
                 print(f"[WARN] Could not dynamically determine feature_dim for {ARCH_NAME} backbone due to: {e}. Defaulting feature_dim to 768.")
                 self.feature_dim = 768 # Typical for convnext_tiny after avgpool
                 # We still need to ensure the final layer is an Identity
                 # This part is tricky if the structure isn't as expected.
                 # Safest might be to remove the entire classifier sequence if it's complex and not just model.classifier[2]
...
def run_augmented_inference_on_dataset(dataset_path, dataset_name_key, arch_name_config, inference_type_tag_config, current_augmented_model_path):
    ...
    try:
        # Assuming knn_feature_dim_global is NUM_CLASSES (probabilities for each class)
        augmented_model_instance = AugmentedConvNeXt(num_classes_global_config=NUM_CLASSES, knn_feature_dim_global_config=NUM_CLASSES)
